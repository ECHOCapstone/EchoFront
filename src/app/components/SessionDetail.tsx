...
import { ArrowLeft, Pencil, Volume2, Home, Mic, User, X, ArrowRight, Check } from 'lucide-react';
...
export default function SessionDetail() {
  ...
  const handleTitleEdit = () => {
    const newTitle = prompt('세션 제목을 입력하세요:', sessionTitle);
  ...
  const handlePlayAudio = () => {
    ...
    console.log('음성 재생:', script);
    alert('음성 재생 기능은 실제 구현 시 TTS API를 사용합니다.');
  };

  const handleSave = () => {
    console.log('세션 저장:', { title: sessionTitle, script });
    setShowRecordModal(false);
  };

  const handleEndLearning = () => {
    if (confirm('학습을 끝내시겠습니까?')) {
      navigate('/custom-learning', { state: { nickname } });
    }
  };
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <div className="flex-1 p-6 pb-24">
        ...
        <div className="mb-4">
          <Textarea
            ...
            className="w-full h-64 p-4 border-2 border-gray-300 rounded-2xl focus:border-sky-500 focus:ring-sky-500 resize-none"
          />
        </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow-lg">
        <div className="flex justify-around items-center h-20">
          ...
          >
            <Mic size={28} />
            <span className="text-xs font-medium mt-1">녹음</span>
          </button>
